...
static GRect current_rect;
static GRect end_rect;
extern tm tm_time;
...
static void grect_setter( void *subject, GRect grect ) {
  #ifdef DEBUG
  ...
  APP_LOG( APP_LOG_LEVEL_INFO, "grect_setter(): %d %d %d %d", grect.origin.x, grect.origin.y, grect.size.w, grect.size.h );
}

static GRect grect_getter( void *subject ) {
  GRect grect = GRect( 0, 0, 0, 0 );
  #ifdef DEBUG
  APP_LOG( APP_LOG_LEVEL_INFO, "grect_getter()" );
  #endif
  HAND_LAYER_DATA *hand_layer_data = ( HAND_LAYER_DATA *) layer_get_data( (Layer *) subject );
  grect = hand_layer_data->current_rect;
  APP_LOG( APP_LOG_LEVEL_INFO, "grect_getter(): %d %d %d %d", grect.origin.x, grect.origin.y, grect.size.w, grect.size.h );
  return grect;
}
...
static void handle_clock_tick( struct tm *tick_time, TimeUnits units_changed ) {
  ...
  if ( tm_time.tm_sec % 60 ) {
    ...
    Animation **digit_animation_array = (Animation**) malloc( ( NUM_DIGITS + 4 ) * sizeof( Animation* ) );
    ...
    Animation *hour_hand_animation = 0;
    
    static const PropertyAnimationImplementation hour_hand_animation_implementation = {
      .base = {
    ...
    HAND_LAYER_DATA *hand_layer_data = ( HAND_LAYER_DATA *) layer_get_data( hour_layer );
    hour_hand_prop_animation = property_animation_create( &hour_hand_animation_implementation, hour_layer,
                                                         &( hand_layer_data->current_rect ), &( hand_layer_data->home_rect ) );
    hour_hand_animation = property_animation_get_animation( hour_hand_prop_animation );
